...
import styles from './Postpartum.less';

const moduleName = modular.getModuleName( modular.postpartum );
...
const Postpartum = ( {
	...
	return (
		<div className={styles.wrap}>
			...
			<div className={styles.header}>预约产后访视委托表</div>
			...
			<Form onSubmit={showModal}>
				...
				<ResidentInfoTable name={modular.postpartum.name} ref={e => ( residentInfoTable = e )}/>
...
const mapDispatchToProps = ( dispatch ) => {
	return {
		onSubmit: ( values ) => {
			...
			dispatch( { type: fun.fuse( model.bookingAgent, action.BA_savePostpartum ), payload: values } );
		},
		changeConfirmState: () => {
		...
		showModal: ( e ) => {
			...
			dispatch( {
				...
				payload: { itemId: modular.postpartum.ritRef, fun: payModal.showModal }
			} );
		},
...
export default connect( mapStateToProps, mapDispatchToProps )( Form.create()( Postpartum ) );
